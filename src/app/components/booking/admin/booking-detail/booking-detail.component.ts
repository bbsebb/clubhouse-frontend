...
export class BookingDetailComponent implements OnInit {
  ...
  onAccept() {
    this.booking$ = this.bookingService.accept(this.booking.id).pipe(
      ...
      tap((booking) => this.activateButton(booking)),
      tap(() =>
        this.snackMessage.notifyFormSubmission([], 'Réservation acceptée')
      ),
      catchError((err) => {
  ...
  onPay() {
    this.booking$ = this.bookingService
      ...
      .pipe(
        ...
        tap((booking) => this.activateButton(booking)),
        tap(() =>
          this.snackMessage.notifyFormSubmission([], 'Réservation payée')
        ),
        catchError((err) => {
          this.snackMessage.notifyFormSubmission([], 'une erreur est survenue');
          throw new Error(err);
  ...
  onValid() {
    this.booking$ = this.bookingService.valid(this.booking.id).pipe(
      ...
      tap((booking) => this.activateButton(booking)),
      tap(() =>
        this.snackMessage.notifyFormSubmission([], 'Réservation validée')
      ),
      catchError((err) => {
        this.snackMessage.notifyFormSubmission([], 'une erreur est survenue');
        throw new Error(err);
  ...
  onRefuse() {
    this.booking$ = this.bookingService.refuse(this.booking.id).pipe(
      ...
      tap((booking) => this.activateButton(booking)),
      tap(() =>
        this.snackMessage.notifyFormSubmission([], 'Réservation refusée')
      ),
      catchError((err) => {
        this.snackMessage.notifyFormSubmission([], 'une erreur est survenue');
        throw new Error(err);
