...
@Component({
  ...
  standalone: true,
    imports: [CommonModule, MatButtonModule, MatCardModule, MatFormFieldModule, MatInputModule, MatOptionModule, MatSelectModule, ReactiveFormsModule,MatSnackBarModule],
...
export class SignUpComponent implements OnInit{
  ...
  onSubmit():void {
    if(this.formSignUp.valid) {
      ...
      this.userService.createUser(userCreateDTO).pipe(

      ).subscribe({
          next: response => {
            this.router.navigate(['/users/signin'])
              .then(r => this.snackBar.open('Inscription enregistrée','Fermer'));
