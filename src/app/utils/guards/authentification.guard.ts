...
export const authentificationGuard: CanActivateFn = (route, state) => {
  ...
  const snackBar: SnackBarMessageService = inject<SnackBarMessageService>(
    SnackBarMessageService
  );
  let guard: boolean = true;
  authService.isAuth.subscribe((isAuth) => {
    if (!isAuth) {
      router.navigate(['admin', 'users', 'signin']);
      guard = false;
    }
    if (
      isAuth &&
      !authService.getRoles().includes('ADMIN') &&
      !authService.getRoles().includes('USER')
    ) {
      snackBar.notifyFormSubmission(
        ['/admin'],
